...
main = tkinter.Tk()
main.title('AI Approaches for Soil Classification in Precision Agriculture')
        ...
        for root, dirs, directory in os.walk(path):
            for j in range(len(directory)):
                ...
                if 'Thumbs.db' not in directory[j]:
                    ...
                    X.append(img_resized.flatten())     # ARRAY
                    # Append the index of the category in categories list to Y
                    Y.append(categories.index(name))    # ARRAY
        X = np.array(X)
        Y = np.array(Y)
        np.save(X_file, X)
        np.save(Y_file, Y)
...
font = ('times', 18, 'bold')
title = Label(main, text='AI Approaches for Soil Classification in Precision Agriculture')
